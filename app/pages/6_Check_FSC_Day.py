"""Page 6 — Check FSC Day.

Verifies whether a resident can appropriately take a full day or half-day
FSC ("Flexible Scholarly/Conference") day away from clinic: are they on an
ambulatory rotation that week, are they free of assist-list/jeopardy duty,
is the specific day/half-day requested not their own continuity clinic
(DOC/Pickett/PRIME), and do they have enough FSC balance left. Read-only
over the real workbooks at Resident_Schedules/, same posture as Pages 4/5 —
never writes back to Excel or db.models, every check logged to audit_log.
"""
...
from real_schedule.checks import check_fsc_day_request
from real_schedule.fsc_tracker import load_fsc_tracker
from real_schedule.master_schedule import load_master_schedule

require_chief_auth()

st.title("Check FSC Day")
st.caption("Verify a proposed FSC day or half-day request against the real, live schedules (read-only).")
...
def _load_all(master_schedule_mtime: float, master_assist_mtime: float, fsc_tracker_mtime: float, ambulatory_mtime: float, week_sheet_name: str):
    del master_schedule_mtime, master_assist_mtime, fsc_tracker_mtime, ambulatory_mtime  # cache-key only
    master_schedule, w1 = load_master_schedule(_MASTER_SCHEDULE_PATH)
    master_assist, w2 = load_master_assist_list(_MASTER_ASSIST_PATH)
    fsc_balances, w3 = load_fsc_tracker(_FSC_TRACKER_PATH)
    ambulatory_week, w4 = load_ambulatory_week(_AMBULATORY_PATH, week_sheet_name)
    return master_schedule, master_assist, fsc_balances, ambulatory_week, w1 + w2 + w3 + w4


try:
    mtimes = tuple(
        os.path.getmtime(p) for p in (_MASTER_SCHEDULE_PATH, _MASTER_ASSIST_PATH, _FSC_TRACKER_PATH, _AMBULATORY_PATH)
    )
except OSError as exc:
...
if st.button("Check this FSC request", type="primary"):
    ...
    audit_record(
        actor=get_actor(),
        action="check_fsc_day_request",
        reason=f"checked FSC request: {resident_name} on {date_} ({portion_label})",
        details=json.dumps(
            {
    ...
    if result.is_clear:
        ...
        st.error("This FSC request has at least one blocking issue — review before proceeding.")
